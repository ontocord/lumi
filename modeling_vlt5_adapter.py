...
from transformers.models.t5.modeling_t5 import (
    T5Block, T5Stack,  T5LayerNorm, T5LayerSelfAttention, T5LayerFF, T5LayerCrossAttention,
...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None, frcnn=None, image_preprocessor=None, task_embed=None):
        super(T5Stack, self).__init__(config, embed_tokens)
        self.config = config
        self.frcnn = frcnn
...
class VLT5(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        if hasattr(config, 'MODEL'):
          frcnn_config = copy.deepcopy(config)
          for key in dir(frcnn_config):
            if key == key.upper():
              val = getattr(frcnn_config, key)
              if type(val) == dict:
                val2 = PretrainedConfig()
                for key2, val3 in val.items():
                  if key2 == key2.upper():
                    setattr(val2, key2,  val3)
                val = val2
                #print (key, '**', val)
                setattr(frcnn_config, key, val)
          #print (frcnn_config)
          self.frcnn = GeneralizedRCNN(frcnn_config)
          self.image_preprocessor = Preprocess(frcnn_config)
        else:
          self.frcnn = None
        ...
        self.encoder = JointEncoder(encoder_config, self.shared, self.frcnn, self.image_preprocessor)
        #------------------#
    ...
    def forward(
        ...
        **kwargs,
        ...
        vis_inputs = encoder_outputs.vis_inputs
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs):
        ...
        kwargs['vis_inputs'] = encoder_outputs.vis_inputs
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs
        ...
        model_kwargs['vis_inputs'] = encoder_outputs.vis_inputs
        expanded_return_idx = (
...
class VLSeq2SeqLMOutput(ModelOutput):
    ...
    vis_inputs: Optional[Tuple[torch.FloatTensor]] = None
