...
def image_from_numpystr(numpystr, dtype="uint8", shape=(75,75,3)):
...
def process_images(filename, en_mdd_file="/content/drive/Shareddrives/ontocord/mdd/en_mdd.txt.gz", cutoff=10000, score_cutoff=0.20, max_img_per_doc=5, trimmed_text_word_len=50):
  ...
  with open(filename, "w") as out:
   with IndexedGzipFileExt("en_mdd.txt.gz") as f:
      for cnt in tqdm.tqdm(range(cutoff)):
        ...
        for didx, text in enumerate(dat):
            ...
            with torch.no_grad():
              ...
              cindices = clip_output['scores'].sort().indices.tolist()
              most_similar_idx = cindices[-1] 
              sim1 = clip_output['scores'][most_similar_idx]
              # clip scores of generated images tend to be lower; this filters out really bad matches
              if person_ner or sim1 > score_cutoff:    
                ...
                text4 = list(set(list(ner_and_verbs.values())))
                ...
                if text4:
                  ...
                  if clip_output is not None:
                    ...
                    out.write(str({'score': sim1.item(), 'matched_sentence': matched_sentence, 'element2text': element2text, \
                              'prev_text': prev_text, 'next_text':  next_text, 'decomposed_image_features': clip_output['decomposed_image_features'].cpu().numpy().tostring(), \
                              'tokens': tokens.tostring(), 'thumbnail': np.array(img).tostring() })+"\n")
