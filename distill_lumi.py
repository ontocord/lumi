...
class LumiMultiTask(VLT5):
    def __init__(self, config):
        super().__init__(config)
        
    def train_step(self, batch, minidalle_teacher, **kwargs):
        task = batch['task']
        if task == 'vqa':
            ...
            # return train step for dalle
            pass
        #TODO - pass in regular t5 for distillation loss
        
    def valid_step(self, batch, minidalle_teacher, **kwargs):
        task = batch['task']
        if task == 'vqa':
            ...
            # return valid step for dalle
            pass
          
    def test_step(self, batch, minidalle_teacher, **kwargs):
        task = batch['task']
        if task == 'vqa':
...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        assert args.backbone == 'ontocord/vlt5'
        model_class = LumiMultiTask

        config = self.create_config()
        self.tokenizer = VLT5TokenizerFast.from_pretrained(args.backbone) #  self.create_tokenizer()

        self.model = self.create_model(model_class, config, **model_kwargs)
        # used to generate the vq ids from a image description
        self.minidalle_teacher = DalleModel.from_pretrained("ontocord/minidalle").eval()
        for param in self.minidalle_teacher.parameters():
        ...
        #note: the vlt5.frcnn model should have requires_grad == False, and should be ignored by the optimizer
        #check this.
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for step_i, batch in enumerate(self.train_loader):
                ...
                # print(batch)
                task = batch['task']
                if task in ('caption', 'refcoco'):
                  if random.randint(0,1) == 0:
                    task = batch['task'] = 'dalle'
                ...
                if self.args.fp16 and _use_native_amp:
                    with autocast():
                        if self.args.distributed:
                            results = self.model.module.train_step(batch, minidalle_teacher=self.minidalle_teacher)
                        else:
                            results = self.model.train_step(batch, minidalle_teacher=self.minidalle_teacher)
                else:
                    if self.args.distributed:
                        results = self.model.module.train_step(batch, minidalle_teacher=self.minidalle_teacher)
                    else:
    ...
    def vcr_evaluate(self, loader):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(iterator):
                ...
                if self.args.distributed:
                    results = self.model.module.valid_step(batch, self.minidalle_teacher)
                else:
