        ...
        for didx, text in enumerate(dat):
            ...
            with torch.no_grad():
              ...
              if sim1 > score_cutoff:    
                ...
                matched_output, cropped_images = get_sent_to_img(matched_sentence, img, get_cropped_images=True)
                  ...
                  (not matched_output['cropped2text'] or any(a for a in matched_output['cropped2text'].values() if a[1] >= score_cutoff)):
                    ...
                    if word_str:
                      ...
                      if matched_output2 and \
                          (not matched_output['decomposed2text'] or any(a for a in matched_output['decomposed2text'].values() if a[1] >= score_cutoff)) and \
                          (not matched_output['cropped2text'] or any(a for a in matched_output['cropped2text'].values() if a[1] >= score_cutoff)) and \
