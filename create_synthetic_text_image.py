...
def get_sent_to_img(matched_sentence, img, other_sent_arr=[], get_cropped_images=False, num_boxes=5, entities=[]):
        ...
        for didx, text in enumerate(dat):
            ...
            with torch.no_grad():
              ...
              if sim1 > score_cutoff:    
                ...
                other_sent_arr=([] if 'network' in matched_sentence else ['diagram of lines and networks']) + ([] if ' clock ' in matched_sentence else ['a clock']) + ([] if 'abstract' in matched_sentence else ['abstract art'])
                matched_output, cropped_images = get_sent_to_img(matched_sentence, img, get_cropped_images=True, other_sent_arr=other_sent_arr )
                if matched_output and matched_output['score'] >= score_cutoff and \
                  ...
                  (not matched_output['cropped2text'] or any(a for a in matched_output['cropped2text'].values() if a[1] >= score_cutoff)):
                    ...
                    word_str = ", ".join([a[0] for a in decomposed2text.values() if a[1] >= score_cutoff])
                    if word_str:
                      ...
                      other_sent_arr=([] if 'network' in matched_sentence else ['diagram of lines and networks']) + ([] if ' clock ' in matched_sentence else ['a clock']) + ([] if 'abstract' in matched_sentence else ['abstract art'])
                      matched_output2, cropped_images = get_sent_to_img(generated_sentence, img, get_cropped_images=True, other_sent_arr=other_sent_arr, \
