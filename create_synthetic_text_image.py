...
def get_sent_to_img(matched_sentence, img, distractors=[], get_cropped_images=False, num_boxes=5, entities=[]):
  ...
  entities = [strip_left_stopwords(e) for e in entities]
  ...
  ner_and_verbs = dict([(strip_left_stopwords(e.text.lower() if len(e.text) < 5 else e.text.lower()[:5]), e.text) for e in doc.ents if len(e.text) > 4] + \
                           ...
                           [(e.lower() if len(e) < 5 else e.lower()[:5], e) for e in entities] + \
        ...
        for didx, text in enumerate(dat):
            ...
            with torch.no_grad():
              ...
              if sim1 > score_cutoff:    
                ...
                distractors=([] if 'network' in matched_sentence else ['diagram of lines and networks']) + ([] if ' clock ' in matched_sentence else ['a clock']) + ([] if 'abstract' in matched_sentence else ['abstract art'])
                matched_output, cropped_images = get_sent_to_img(matched_sentence, img, get_cropped_images=True, distractors=distractors )
                if matched_output and not matched_output2['distractor_is_best_match'] and matched_output['score'] >= score_cutoff and \
                  ...
                  (not matched_output['cropped2text'] or any(a for a in matched_output['cropped2text'].values() if a[1] >= score_cutoff)):
                    ...
                    word_str = ", ".join([a[0] for a in decomposed2text.values() if a[1] >= score_cutoff and a[0] not in distractors])
                    if word_str:
                      ...
                      matched_output2, cropped_images = get_sent_to_img(generated_sentence, img, get_cropped_images=True, distractors=distractors, \
                      ...
                      if matched_output2 and not matched_output2['distractor_is_best_match'] and \
