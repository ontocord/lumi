...
def create_qa_vlt5(matched_output, img, score_cutoff, aug2ent, max_qa=3, potential_qa_list=[]):
  ...
  if person:
      ...
      if answer not in ("true", "false", "yes", "no") and (random.randint(0,2)==0 or answer not in ("nothing", "nowhere", "unknown", "black", "white")): 
        matched_output['qa'] = matched_output.get('qa',[]) +  [(person, f"what is the {person} feeling?|| {answer}")]    
  entity_to_qa = 0    
              ...
              if sim1 > score_cutoff:    
                ...
                if matched_output and matched_output['decomposed2text'] and matched_output['cropped2text']:
                    ...
                    if word_str:
                      ...
                      if matched_output2 and matched_output['decomposed2text'] and matched_output['cropped2text'] and matched_output2['score'] >= mult*score_cutoff and len([a for a in matched_output2['decomposed2text'].values() if a[1] >= score_cutoff]) >= (len(matched_output2['decomposed2text'])*.5):
