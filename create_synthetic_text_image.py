...
def get_decomposed_sent_to_img(matched_sentence, img, other_sent_arr=[], get_cropped_images=False):
  ...
  if text4:
    if get_cropped_images:
      normalized_boxes = decode_image(asarray(img), vlt5.frcnn,  vlt5.image_preprocessor, max_detections=len(text4)*2)["normalized_boxes"][0]
        ...
        for didx, text in enumerate(dat):
            ...
            with torch.no_grad():
              ...
              if sim1 > score_cutoff:    
                ...
                matched_output = get_decomposed_sent_to_img(matched_sentence, img, [vlt5_caption], get_cropped_images=True)
                if matched_output:
                    ...
                    if word_str:
                      ...
                      matched_output2 = get_decomposed_sent_to_img(generated_sentence, img, get_cropped_images=True)
